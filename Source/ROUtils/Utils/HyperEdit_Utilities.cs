...
namespace ROUtils
{
    ...
    public static class HyperEdit_Utilities
    {
        ...
        public static void DoAirlaunch(double distanceFromKSC, double azimuth, double flightHeading, double altitude, double velocity)
        {
            CelestialBody body = FlightGlobals.currentMainBody;
            Vessel vessel = FlightGlobals.ActiveVessel;

            // SpaceCenter.Instance.transform doesn't work correctly with RSS + KSCSwitcher installed
            PQSCity ksc = KSPUtils.FindKSC(body);
            ...
            var δ = distanceFromKSC / body.Radius;
            var d = kscNorth * Math.Cos(Mathf.Deg2Rad * azimuth) + kscEast * Math.Sin(Mathf.Deg2Rad * azimuth);
            var teleportPosition = kscPosition * Math.Cos(δ) + body.Radius * d * Math.Sin(δ);
            teleportPosition += teleportPosition.normalized * (altitude - kscAltitude);
            ...
            teleportVelocity += (Math.Sin(Mathf.Deg2Rad * flightHeading) * east + Math.Cos(Mathf.Deg2Rad * flightHeading) * north) * velocity;
            ...
            rotation *= Quaternion.AngleAxis((float)flightHeading, Vector3.back);
