...
namespace ROUtils.DataTypes
{
    public class PersistentCompressedConfigNode : IConfigNode
    {
        ...
        private static bool _NeedCheckSettings = true;
        private static bool _LeaveUncompressed = false;
        public static bool LeaveUncompressed
        {
            get
            {
                if (_NeedCheckSettings)
                {
                    _NeedCheckSettings = false;
                    foreach (ConfigNode n in GameDatabase.Instance.GetConfigNodes("ROUtilsSettings"))
                    {
                        n.TryGetValue("doNotCompressData", ref _LeaveUncompressed);
                    }
                }
                return _LeaveUncompressed;
            }
            set
            {
                _LeaveUncompressed = value;
            }
        }
        ...
        protected bool Decompress()
        {
            ...
            Profiler.BeginSample("ROUtilsDecompress");
        ...
        {
            Profiler.BeginSample("ROUtilsCompress");
        ...
        {
            if (_node == null || LeaveUncompressed)
                return;
        ...
        protected unsafe void LoadData(string s)
        {
            ...
            if (mod != 0)
            {
                Debug.LogError("error: base64 string length is not divisble by 4! Padding.");
    ...
    public class PersistentCompressedCraftNode : PersistentCompressedConfigNode
    {
        ...
        public bool GetAndUpgradeNode()
        {
            if (_node == null)
            {
                ...
                if (!runSuccess)
                {
                    Debug.LogError($"Error upgrading craft node with ship name {_node.GetValue("ship") ?? "<unknown"}. Information from the upgrade run: {runInfo}");
                    return false;
                }
                ...
                {
                    base.Node = newNode;
                    Debug.Log($"Upgraded craft node with ship name {_node.GetValue("ship") ?? "<unknown>"}.");
                    return true;
                }
            }
            return false;
